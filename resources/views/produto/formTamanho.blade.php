...
<form action="{{ ($produtoTamanho)? route('produto.updateTamanho'):
...
<div class="row">
    <div class="col-md-3">
        ...
        @selected(($produtoTamanho && $produtoTamanho->id_tamanho == $item->id_tamanho)?true : false)
        >
            {{$item->tamanho}}
        </option>
        @endforeach
    ...
    <div class="col-md-3">
        ...
        value="{{($produtoTamanho)?$produtoTamanho->preco
        : old('preco')}}" required>
    </div>
    ...
    <div class="col-12">
        ...
        <textarea class="form-control" name="observacoes" id="observacoes"
         cols="30" rows="10">{{($produtoTamanho)? $produtoTamanho->observacoes: old('observacoes') }}</textarea>
    </div>
</div>

    @if ($produtoTamanho)
        <input class="btn btn-primary" type="submit" value="Atualizar Tamanho Produto">
    @else
